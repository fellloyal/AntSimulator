...
export class FoodPileRenderer {
  ...
  drawFoodPiles(
    ...
    // UI美化：按光栅顺序遍历未绘制的食物单元，每个都找最大 NxN 块，画一个尺寸 = N×cellSize 的精灵
    // 修正：移除"锚点"限制（否则相邻块会被漏掉），
    //      块大小扫描改为检查新增的整行整列而不是对角线或同列
    ...
    for (let y = sy; y <= ey; y++) {
      for (let x = sx; x <= ex; x++) {
        ...
        let blockSize = 1;
        while (x + blockSize < world.map.width && y + blockSize < world.map.height) {
          let allFood = true;
          // 检查新行
          for (let bx = 0; bx <= blockSize; bx++) {
            const c = foodMap.get((y + blockSize) * world.map.width + (x + bx));
            if (!c || c.type !== type) { allFood = false; break; }
          }
          if (!allFood) break;
          // 检查新列
          for (let by = 0; by <= blockSize; by++) {
            const c = foodMap.get((y + by) * world.map.width + (x + blockSize));
            if (!c || c.type !== type) { allFood = false; break; }
          }
          if (!allFood) break;
          blockSize++;
        ...
        for (let by = 0; by < blockSize; by++) {
          for (let bx = 0; bx < blockSize; bx++) {
            drawn.add((y + by) * world.map.width + (x + bx));
          }
        }
