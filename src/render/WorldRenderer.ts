...
export class WorldRenderer {
  ...
  render(ctx: CanvasRenderingContext2D, _viewport: ViewportState): void {
    ...
    // Only update dirty cells instead of all 129,600 cells
    const dirtyList = this.map.dirtyList;

    if (dirtyList.length > width * height * 0.5) {
      // More than 50% dirty — just update everything
      data.fill(0);
      for (let i = 0; i < cells.length; i++) {
        this.renderCell(data, cells, i, numColonies, colonyRgb, intensityFactor, drawMarkers, drawDensity);
      }
    } else {
      // Update only dirty cells
      for (let j = 0; j < dirtyList.length; j++) {
        const idx = dirtyList[j];
        this.renderCell(data, cells, idx, numColonies, colonyRgb, intensityFactor, drawMarkers, drawDensity);
      }
    }
  ...
  private renderCell(
    ...
    cells: import('@/simulation/types').WorldCell[],
    cellIdx: number,
    numColonies: number,
    colonyRgb: Array<{ r: number; g: number; b: number }>,
